...
class ReportPerformanceComputorConfig:
    # llm
    base_url: str = "http://172.16.2.88:32521/v1"
    api_key: str = "EMPTY"
    model: str = "qwen"
...
class ReportPerformanceComputor:
    ...
    def compute_performance(self, df_gt: pd.DataFrame, df_pred: pd.DataFrame):
        ...
        macro_recall = round(df_valid["recall"].mean(), 4)
        macro_precision = round(df_valid["precision"].mean(), 4)
        macro_f1 = round(df_valid["f1"].mean(), 4)
        
        # Micro mean: 所有类别的TP、FP、FN、TN加总后计算
        total_tp = df_valid["tp"].sum()
        total_fp = df_valid["fp"].sum()
        total_fn = df_valid["fn"].sum()
        total_tn = df_valid["tn"].sum()
